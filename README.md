...
## 📴 Запуск без БД (in-memory)
Если хотите просто развернуть и проверить приложение без PostgreSQL:

- PowerShell (текущая сессия):
  - `$env:DISABLE_DB = "true"`
  - `npm run dev`  (или отдельные: `node scripts\start-server-dev.js` и `node scripts\start-client-dev.js`)

- CMD (текущая сессия):
  - `set DISABLE_DB=true`
  - `npm run dev`

В этом режиме сервер использует in-memory хранилище, совместимое с текущими эндпоинтами. При перезапуске данные сбрасываются.
...
## 🔐 OpenSSL/webpack ошибка на новых Node
При Node ≥ 17 возможна ошибка `ERR_OSSL_EVP_UNSUPPORTED` от Webpack 4.
...
- Рекомендованное решение: используйте Node 16.20.x (через nvm-windows).

После установки/переключения Node перезапустите терминал, затем:
- `npm ci`
- `npm run dev`
